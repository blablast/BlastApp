tautologies_p = [
    ("1. Prawo niesprzeczności", "(~(p AND ~p))"),
...
tautologies_pq = [
    ...
    (
        "17. Pierwsze prawo definiowania alternatywy wykluczającej",
        "(q XOR p) <=> ((q AND (~p)) OR ((~q) AND p))",
    ),
    (
        "18. Drugie prawo definiowania alternatywy wykluczającej",
        "(q XOR p) <=> ((q OR p) AND (~(q AND p)))",
    ),
...
tautologies_pqr = [
    ("24. Prawo przechodniości implikacji", "((r => q) AND (q => p)) => (r => p)"),
...
tautologies_pqrs = [
    (
        "30. Prawo łączenia alternatywnego stronami",
        "((s => r) AND (q => p)) => ((s OR q) => (r OR p))",
    ),
    (
        "31. Prawo łączenia koniunkcyjnego stronami",
        "((s => r) AND (q => p)) => ((s AND q) => (r AND p))",
    ),
]
